...
Control Plane Harness is a transactional local control plane for one Master
...
```text
<python> -B tools/package_manager.py install --package orchestration --version 0.2.3 --root "<install-root>" --dry-run
<python> -B tools/package_manager.py install --package orchestration --version 0.2.3 --root "<install-root>" --apply
<python> -B tools/package_manager.py verify --package orchestration --version 0.2.3 --root "<install-root>"
```

The installed copy is `<install-root>/orchestration-0.2.3/`. The common package
...
```text
<python> -B hq.py --root "<workspace>" --json init --id sample-front --name "Sample Front" --path fronts/sample-front --alias sample --dry-run
```
...
<python> -B scripts/structural_check.py
```
...
<python> -B hq.py --root "<workspace>" --json bom-dia
<python> -B hq.py --root "<workspace>" --json init --id sample-front --name "Sample Front" --path fronts/sample-front --alias sample --dry-run
<python> -B hq.py --root "<workspace>" --json init --id sample-front --name "Sample Front" --path fronts/sample-front --alias sample --apply
<python> -B hq.py --root "<workspace>" --json hq-sync
<python> -B hq.py --root "<workspace>" --json foco sample-front
<python> -B hq.py --root "<workspace>" --json digere --summary "Validated the current slice" --pending "Register the validated delta"
<python> -B hq.py --root "<workspace>" --json registra --note "Evidence recorded"
<python> -B hq.py --root "<workspace>" --json encerra --summary "Closed the work block" --next "Review the next bounded slice"
<python> -B hq.py --root "<workspace>" --json bom-dia
```
...
```text
<python> -B hq.py --root "<workspace>" --json recover --apply
```
...
<python> -B hq.py --root "<workspace>" --json repair-panel --apply
```
...
- Immutable documentation URL: https://github.com/fabianomag/agent-harnesses/blob/v0.2.3/packages/orchestration/README.md
- Immutable install prompt: `Install this harness for me: https://github.com/fabianomag/agent-harnesses/releases/download/v0.2.3/orchestration-0.2.3.zip`
- Immutable source URL: https://github.com/fabianomag/agent-harnesses/tree/v0.2.3/packages/orchestration
...
- Immutable spec: https://github.com/fabianomag/agent-harnesses/blob/v0.2.3/graphs/orchestration.graph.json
