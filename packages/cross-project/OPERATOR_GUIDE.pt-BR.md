...
- Receipt: path relativo ao target `.agent-harnesses/runtime/cross-project/0.2.3/.agent-harness-receipt.json`.
- A partir de um bundle `0.2.3` com checksum verificado, antecipe a remoção do package com `<python> -B installer.py uninstall cross-project --target "<target>" --dry-run --json`; após a revisão, aplique-a com `<python> -B installer.py uninstall cross-project --target "<target>" --apply --json`.
